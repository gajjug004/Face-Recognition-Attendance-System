#!D:\Minor Project\Face-Recognition-Based-Attendance-System\.venv\Scripts\activate
import dlib
import numpy as np
import cv2
import os
...
model = YOLO("../model/yolov8_best2.pt")
...
class Face_Recognizer:
    def __init__(self):
        ...
        #  cnt for real faces in frame N-1 and N
        self.last_frame_real_face_cnt = 0
        self.current_frame_real_face_cnt = 0
    ...
    def draw_note(self, img_rd):
        ...
        cv2.putText(img_rd, "FPS:    " + str(self.fps.__round__(2)), (20, 130), self.font, 0.8, (0, 255, 0), 1,
                    cv2.LINE_AA)
        cv2.putText(img_rd, "Real Faces:  " + str(self.current_frame_real_face_cnt), (20, 160), self.font, 0.8, (0, 255, 0), 1,
                    cv2.LINE_AA)
    ...
    def process(self, stream):
        ...
        if self.get_face_database():
            while stream.isOpened():
                ...
                results = model(img_rd, stream=True, verbose=False)
                real_count = 0
                for r in results:
                    boxes = r.boxes
                    for box in boxes:
                        ...
                        if conf > 0.5:
                            ...
                            cvzone.cornerRect(img_rd, (x1, y1, w, h), colorC=color, colorR=color)
                            cvzone.putTextRect(img_rd, f'{classNames[cls].upper()} {int(conf * 100)}%',
                                            (max(0, x1), max(35, y1)), scale=2, thickness=4, colorR=color,
                                            colorB=color)

                # 3.5  Update cnt for real faces in frames
                self.last_frame_real_face_cnt = self.current_frame_real_face_cnt
                self.current_frame_real_face_cnt = real_count
